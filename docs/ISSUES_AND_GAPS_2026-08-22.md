...
| E12 | **RUN 2026-09-06/07 — E12, E15 and E16 all have results; see `docs/SWA_BOARD_2026-08-31.md` §"What the board found".** E12 (35 jobs) and E15 (30 jobs) turned out to be complete on disk and never read; E16's grid ran 2026-09-07 (225 runs). **The window is a cliff on recall, not a tax.** CE degrades smoothly as the window narrows (ctx 512: w256 +0.0050, w128 +0.0365, w64 +0.0986 nats vs dense, every comparison 5/5); recall does not degrade at all. At seq 255 dense attention solves 11/15 [0.48, 0.89] and every 64-windowed arm solves **0/15** [0.00, 0.20]; at seq 511, 4/15 against 0/15. Disjoint intervals, 60/60 windowed runs fail. Consequence for the hybrid: windowing `hybrid_mingru10_swa2`'s two attention layers costs **+0.0498 ± 0.0037 nats, 5/5**, against a hybrid advantage of 0.0176 — so **the hybrid's attention must be global.** At ctx 2048 a quarter-context window (`swa_w512`) ties dense outright (−0.0017 ± 0.0087, 2/5) but costs 50% wall clock. Unexpected side result: removing attention **sinks is BETTER** on CE — 0.0083 ± 0.0014 at ctx 512 and 0.0112 ± 0.0009 at ctx 2048, 10/10 seed-pairs. Original row follows. **CODE COMPLETE 2026-08-31, unrun.** The claim this row carried — "no windowed-attention path exists in `mixers.py`" — has been false since the `swa` mixer landed: `swa` is in the `MIXERS` registry, `Attention` takes `window`/`sinks`, and `swa_chunked` is pinned identical to the dense window by `nanolab/tests.py`. **Sliding-window attention arm** — asks whether the hybrid's attention layers must be global (the QSA question at small scale). Superseded in detail by `docs/SWA_BOARD_2026-08-31.md`, which specifies E12 together with the two items it motivated: **E15** (the same arms at ctx 2048) and **E16** (MQAR across lengths that straddle the window). Both arms exist — `swa_w64/w128/w256` and `hybrid_mingru10_swa2`. Gate satisfied (E8 ran). | GH200 jobs; no code |
| E17 | **RUN 2026-09-06 — it compiles, it is correct, and it does NOT beat SDPA for training.** First `nvcc` build and first execution; the tensor-core layout probe passed against the device. At B8/T1024/H12-4/D32 bf16: SDPA 0.193 ms fwd / 0.446 ms fwd+bwd; the mma path **0.143 ms fwd (1.35x faster)** but **0.468 ms fwd+bwd (0.95x — 5% SLOWER)**; the FMA path is 0.23x fwd and ~7x slower fwd+bwd. So the forward win does not survive the backward, and this lab trains. The row's demand — "no throughput number … and none should be quoted until there is" — is now satisfied, and the answer to "does it beat what is already there" is **no at this shape**. Caveat: ONE shape, and not this lab's (`d_model` 768, `head_dim` 64, ctx 512); re-run at the board shape before concluding generally. Original row follows. **CODE COMPLETE 2026-09-02 — UNCOMPILED AND UNRUN.** `nanolab/csrc/flash_attn_cuda.cu` is an FA-2 causal GQA kernel for CUDA — forward and backward, an FMA path and a tensor-core path (`mma.sync.aligned.m16n8k16`, `cp.async` double-buffered) — opt-in behind `--flash_cuda` and off by default. It has never been through `nvcc` and has never executed. **There is no throughput number for it anywhere in this repository, and none should be quoted until there is.** What *is* established, and entirely on CPU: `csrc/flash_attn_sim.py` mirrors every kernel's index arithmetic — the tensor-core ones at lane granularity, each lane holding the fragment elements the PTX layout gives it — and matches brute-force attention and its autograd gradients to **≤6e-15 in float64** across head_dim 32/64/128, T not tile-aligned, and GQA groups 1/2/4. Two assumptions the mirror cannot reach are handled unequally, on purpose: the mma fragment layout is *deterministically* wrong if it is wrong, so `mma_probe` multiplies known matrices through the kernel's own fragment helpers on the device and the tensor-core path stays off unless that agrees with a plain matmul; cp.async synchronisation is a *race* if it is wrong, and a race passes a probe by luck, so the only available check is that a stale tile breaks run-to-run reproducibility. Note also that the incumbent is not a straw man — SDPA on CUDA already reaches a CUTLASS FlashAttention-2 — so the open question is not "does it work" but "does it beat what is already there, at any shape this lab runs". **The asymmetry with the Metal side is the sharpest way to state this gap:** the same algorithm on Apple silicon has a published measurement set (`Rust_MLKit/crates/tessl/bench/results/attn_*_m5pro.json`, commit `0623c61`); the CUDA port has none. Spec: backlog E17. | minutes on any CUDA box |
| E27 | **IN FLIGHT 2026-09-05 — the width-1536 rung of the ladder.** Overnight run orphaned (two jobs left `running`, probe with two CUDA-OOM cells at `workers: 2`); repaired the same morning; the minGRU probe now resolves to lr40 (interior; lr20 tied within the 0.0014-nat rerun floor) and the minGRU arm runs at `workers: 1` in its own directory; four of five attention jobs OOMed a second time when the repair collided with a tuning stage. One relaunch left. State and steps: handoff §2.2. | ~1.5 GH200-h |
| E28–E35 | **REGISTERED 2026-09-05, coded (`1b6c5d0`), unrun.** GDN-rule recall ablation, MoE with a task-trained router, within-suite and near-parity closers on the 8+4 hybrid, tied/untied × value residual, 8+4 on recall, depth-for-width, the copy-loss probe, the width-384 token ladder. Priced from measured per-arm cost, not elapsed ÷ tenancy: ~17 GPU-h with 3000-step recall cells, ~34 with the 9000-step cells too. `docs/architecture-review-2026-09-04/LAMBDA_HANDOFF_2026-09-05.md`. | 17–34 GH200-h |
...
| **D15** | **The GDN operator is a variant of the published one** (found 2026-09-04 by the architecture review, reproduced on the real functions). `gdn_chunked` and `_sequential` compute the delta correction from the undecayed state; arXiv 2412.06464 eq. 8 uses the decayed state. The regression test's reference implements the same variant, so it could not see it. Every GDN number in the record is the repo variant. | **FLAGGED 2026-09-05** — `Config.gdn_rule` (`repo` default, `published`), arms `gdn_pub`/`hybrid_gdn_periodic_pub`, both forms tested against each other under both rules. Ablation is E28; the default is an open decision. |
| **D16** | **Top-1 MoE routers received no task gradient.** `weights / weights.sum()` is identically 1 at top-1, so `moe_e4k1`/`moe_e8k1` (moe32b, moe32c) trained a balance-only router (task-gradient norm 3e-18 vs 0.12 from the aux). | **FLAGGED 2026-09-05** — `Config.moe_router_weight` (`renorm` default, `raw` = Switch-style), arms `moe_e{1,4,8}k1_raw`; test pins both behaviours. Re-run is E29; `docs/MOE_BOARD_2026-09-04.md` carries the banner. |
| **D17** | **The sampler switches token streams on free VRAM** (tuning sprint, 2026-09-05). `data.should_gpu_resident` takes the GPU-resident path (CUDA generator) only above 9.27 GiB free at `Batcher` construction and otherwise the memmap path (CPU generator): same seed, different tokens, no warning. Construction precedes model build, so a late-starting co-resident worker is the one at risk, and the fallback needs *less* VRAM, so it converts an OOM into a run that trains on different data. | **open** — no committed run is known to have crossed the line (every tenancy on disk leaves > 30 GiB free at construction), but the guard should fail loud, not fall back. `docs/GPU_TUNING_2026-09-05.md` §4. |
...
it would work, and enabling it would be one more recipe change. *(2026-09-05: the stall
does not reproduce on torch 2.7.0 — attention compiles in 31 s at 1.94x, a pure minGRU
stack at 1.96x. The second sentence stands: `compile` is now a recorded recipe field,
default off, and turning it on shifts `final_val` by ~0.0023 nats.)*
