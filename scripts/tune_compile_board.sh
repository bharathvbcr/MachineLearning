...
#
# Retained from the earlier version of this script, because the branch that
# replaced it dropped the point and the point still holds: speed alone cannot
# decide this. Inductor fuses and re-associates, so a compiled run's loss moves
# in the last places, and any board pooling with an existing directory would be
# comparing two numerics. `compile` is a recorded recipe field, so the eager and
# compiled directories can never be merged by accident.
set -u
cd ~/MLSystemsLab
