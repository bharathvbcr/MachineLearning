...
- **[`nanolab/`](nanolab/)** — a clean, instrumented small-LM lab where architectural and training choices are exposed as CLI flags, with attention, sliding-window attention, Mamba-2, Gated DeltaNet, minGRU, hybrid interleavings, SFT, STaR, and diffusion experiments, plus a multi-query associative-recall probe (`nanolab/mqar.py`) for the in-context retrieval axis that held-out cross-entropy does not exercise. [`nanolab/csrc/`](nanolab/csrc/) adds an opt-in custom CUDA flash-attention kernel (`--flash_cuda`) ported from the Metal kernels — forward and backward, FMA and `mma.sync` tensor-core paths — whose arithmetic is verified on CPU against a brute-force reference to ≤6e-15 in float64 but which **has never been compiled or run**; there is no throughput number for it, and gap E17 in [`docs/ISSUES_AND_GAPS_2026-08-22.md`](docs/ISSUES_AND_GAPS_2026-08-22.md) is the reason none should be quoted.
...
- **[Architecture research review](docs/architecture-review-2026-09-04/README.md)** preserves the supplied memos, experiment evidence, corrections, and memory-update protocol. The **[September 13 deeper audit](docs/architecture-review-2026-09-04/DEEP_AUDIT_2026-09-13.md)** provides the current assessment: newer hybrid recall and CE results, reproduced resume and analysis-reader defects, scaling qualifications, close prior art, and revised research priorities, with saved results and executable CPU checks.
