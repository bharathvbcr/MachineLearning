...
//!
//! Training encode is **Metal 4 only**: one `MTL4CommandBuffer` per step with
//! argument-table binds, a bump-allocated const arena (~1 MiB), residency
//! registry, and SharedEvent sync. Steady-state training never host-waits except
//! at log / loss / eval boundaries via [`GpuRuntime::synchronize`].
//!
//! Audit 4: cold-buffer recycle + `removeAllocation` after CB complete; one
//! compute encoder packed across `with_binder` calls (P1); working-set probe.
...
use objc2_metal::{
    ...
    MTL4CommandEncoder, MTL4ComputeCommandEncoder, MTL4CommandQueue, MTL4CounterHeap,
    MTL4CounterHeapDescriptor, MTL4CounterHeapType, MTL4TimestampHeapEntry, MTL4VisibilityOptions,
    MTLAllocation, MTLBuffer, MTLComputePipelineState, MTLCreateSystemDefaultDevice, MTLDevice,
    MTLEvent, MTLLibrary, MTLResidencySet, MTLResidencySetDescriptor, MTLResourceOptions,
    MTLSharedEvent, MTLSize, MTLStages,
};
...
const METAL4_CONST_ARENA_BYTES: usize = 1024 * 1024;
...
impl PipelineCache {
    ...
    fn get_or_create(
        ...
        self.map.insert(name.to_string(), pipeline.clone());
        Ok(pipeline)
    }
}
...
impl BufferPool {
    ...
    fn bucket(nbytes: usize) -> usize {
        // Power-of-two buckets are useful for small, short-lived tensors, but
        // waste hundreds of MiB per persistent bank at 128M scale.  Above
        // 1 MiB, use 4 MiB size classes: enough reuse for tape buffers while
        // bounding internal fragmentation to <4 MiB per allocation.
        const SMALL_LIMIT: usize = 1024 * 1024;
        const LARGE_GRANULE: usize = 4 * 1024 * 1024;
        let nbytes = nbytes.max(256);
        if nbytes <= SMALL_LIMIT {
            nbytes.next_power_of_two()
        } else {
            nbytes.saturating_add(LARGE_GRANULE - 1) / LARGE_GRANULE * LARGE_GRANULE
        }
    }
    ...
    fn recycle(&mut self, buffer: Retained<ProtocolObject<dyn MTLBuffer>>) {
        ...
        if self.cached_bytes + key > self.max_cache_bytes {
...
/// Metal 4 encode package (queue / allocator / argument table / CounterHeap).
...
/// Shared GPU runtime for the metal-native crate (Metal 4 encode required).
...
impl GpuRuntime {
    ...
    pub fn from_metallib_path(path: &Path) -> Result<Arc<Self>, String> {
        ...
        // Metal 4 encode package is required (DECISIONS M3: Metal4-only).
        let metal4 = try_init_metal4(&device).map_err(|err| {
            format!(
                "Metal 4 encode package unavailable ({err}); metal-native requires Metal 4"
            )
        })?;
    ...
    pub fn register_residency(&self, buf: &ProtocolObject<dyn MTLBuffer>) {
        let m4 = &self.metal4;
        m4.residency
            .addAllocation(ProtocolObject::<dyn MTLAllocation>::from_ref(buf));
    ...
    pub fn pipeline(
        ...
        cache.get_or_create(&self.device, &self.library, name)
    }
    ...
    /// Ensure a bump slab of at least `capacity` bytes (large allocations use
    /// the pool's bounded-fragmentation size classes).
    pub fn ensure_bump(self: &Arc<Self>, capacity: usize) -> Result<(), String> {
        let cap = BufferPool::bucket(capacity);
        ...
        // Recycle the previous slab into the pool before replacing.
        if let Some(old) = bump.take() {
            self.unregister_residency(old.buffer.metal());
            if let Ok(mut pool) = self.pool.lock() {
                pool.recycle(old.buffer.inner.buffer.clone());
            }
        }
    ...
    /// Reset bump cursor to 0. Caller must ensure no in-flight GPU work still
    /// references bump views (typically after [`Self::synchronize`]).
    ...
    fn ensure_m4_cb_open(&self, batch: &mut Option<ActiveMetal4Batch>) -> Result<(), String> {
        ...
        if need_begin {
            m4.command_buffer
                .beginCommandBufferWithAllocator(&m4.allocator);
            m4.command_buffer.useResidencySet(&m4.residency);
    ...
    fn commit_m4(&self, wait: bool) -> Result<(), String> {
        ...
        if wait {
            ...
            // Allocator may be reused only after the CB completes.
            m4.allocator.reset();
            if write_t1 {
                if let Some(heap) = m4.counter_heap.as_ref() {
...
fn try_init_metal4(
    ...
    let counter_heap = match device.newCounterHeapWithDescriptor_error(&heap_desc) {
        Ok(h) => Some(h),
        Err(e) => {
            eprintln!("[metal-native] MTL4CounterHeap unavailable ({e}); timestamps off");
            None
        }
    };
