...
| [37-golden-token-parity](gemma-metal/37-golden-token-parity.md) | greets + intermediates vs MLX | resolved | High | Apple M5 Pro · mlx golden vs gemma-metal Hot 31B | Resolved 2026-07-19 under capture-on: NeoX proportional RoPE + required hidden capture → `target_next=531`, **greet16 = 16/16**. The historical 0/16 is the pre-fix baseline. Remaining speculative blocker is draft-accept, not Hot golden tokens. |
...
- ~~Extend native golden capture past the current 0/16 greet mismatch~~ — **closed 2026-07-19**: greet16 is 16/16 under capture-on (suite 37). The open remainder is narrower: extend capture to target-hidden, fc-out, h-context, and proposed-token checkpoints so the *draft-accept* path gets the same treatment the greedy path now has.
