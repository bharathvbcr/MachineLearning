...
and **last of four** on the `exact_128m` configuration, 128.37M parameters
(2.950696); the eventual
...
| 7 | minGRU | 4.449 | [4.423, 4.476] |
| 8 | hybrid Mamba×10 + attn×2 | 4.604 | [4.517, 4.692] |
| 9 | MLA | 4.606 | [4.579, 4.634] |
| 10 | Mamba-2 | 4.656 | [4.587, 4.726] |

Not every arm is measured at exactly the same horizon. These runs used two
evaluation cadences — six arms are evaluated on a grid whose last point falls
at 49.99M tokens, four on one ending at 49.17M — so "at 50M tokens" means
within 1.6% of it, not identically. The gap between adjacent ranks is larger
than the drift a 0.82M-token difference produces, so the ordering is not an
artifact of it, but the table is not a single-horizon comparison and should not
be read as one.
...
**A second correction to this table, 2026-08-23.** Mamba-2 was previously
listed at 4.759 [4.680, 4.837]. That figure was not a 50M-token number: it was
measured at **40.16M**. Three of Mamba-2's five seeds ran on the 49.17M
evaluation grid and two on the 49.99M one, and the `summary.json` the table was
built from had been generated by an older aligner that took the *exact
intersection* of the two grids. For this arm the intersection is three points,
ending at 40.16M — so one row of a ten-arm board was reported at 80% of the
horizon of the other nine, with nothing in the table saying so.

The aligner was fixed to map seeds onto the reference grid at-or-after each
target, which is what `nanolab/crossover_replicate.py` does now; the artifact
was simply never regenerated, so the manuscript kept quoting the stale file.
Regenerating it gives **4.656 [4.587, 4.726]** at 49.17M. Mamba-2 stays last
and no rank anywhere on the board moves — the arm was already far enough behind
that a 20% horizon error did not change its position, which is exactly why the
error was survivable long enough to reach a manuscript.
...
- **MONA** ranks **first** at 16M parameters (2.180898 BPB, two seeds) and
  **last of four** on the `exact_128m` configuration (2.950696), a
  **128,367,988**-parameter model. "Exact" there names the funnel's
  bit-exactness gate -- a checkpoint resume reproducing loss, gradient norm and
  sampled weights to within ~2.6e-06, recorded in
  `research/exact-128m-gate-polar.json` -- not a round parameter count. Earlier
  drafts of this paper wrote "an exact 128M-parameter model", which reads as a
  claim about the parameter count and is wrong by 0.37M.
